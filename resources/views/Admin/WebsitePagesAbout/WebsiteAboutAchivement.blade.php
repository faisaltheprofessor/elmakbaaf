...
<div class="border-collapse w-full flex flex-col pt-4 bg-gray-100">
    ...
    <form class="p-10 ml-5 mr-5 bg-white rounded shadow-xl" method="Post" enctype="multipart/form-data" action="/websiteAboteAchivement" >
      @csrf
      ...
      <div class="inline-block mt-2 w-1/4 pr-1">
          ...
          <input   class="w-full px-2 py-2 text-gray-700 " id="" name="titale" type="text" required="" placeholder="" >
      </div>
      <div class="inline-block mt-2 -mx-1 pl-1 w-1/4">
          <label class=" block text-sm text-gray-600" for="">Year</label>
          <input   class="w-full px-2 py-2 text-gray-700 " id=""  name="year" type="text" required="" placeholder="" >
      </div>
      <div class="inline-block mt-2 w-1/4 pr-1">
          <label class=" block text-sm text-gray-600" for="">Achivement</label>
          <input   class="w-full m-2 px-2 py-2 text-gray-700 " id="" name="achivement" type="text" required="" placeholder="" >
      </div>
      <div class="inline-block mt-2 -mx-1 pl-1 w-1/4">
          <label class=" block text-sm text-gray-600" for="">Description</label>
          <input   class="w-full m-2 px-2 py-2 text-gray-700 " id=""  name="description" type="text" required="" placeholder="" >
      </div>
...
<div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 pr-10 lg:px-8">  
    ...
    <thead>
        <tr>
            ...
            <th class="p-3 font-bold uppercase  text-blue-500 border border-gray-300 hidden lg:table-cell">Titale</th>
            <th class="p-3 font-bold uppercase  text-blue-500 border border-gray-300 hidden lg:table-cell">Year</th>
            <th class="p-3 font-bold uppercase  text-blue-500 border border-gray-300 hidden lg:table-cell">Achivement</th>
            <th class="p-3 font-bold uppercase  text-blue-500 border border-gray-300 hidden lg:table-cell">Description </th>
    ...
    @foreach ($achivements as $achivement) 
        <tr class="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">                   
           ...
           <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
            <span class="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold uppercase">Titale</span>
            {{$achivement->titale}} 
           </td>
  
           <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
            <span class="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold uppercase">Year</span>
           {{$achivement->year}}
           </td>
  
           <td class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
            <span class="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold uppercase">Achivement</span>
          ...
          <td class="w-full  lg:w-auto p-3 text-gray-800 text-center border border-b block lg:table-cell relative lg:static">
              <div class="flex ml-8">   
                   <a href="{{url('/websiteAboteAchivement/edit',$achivement->id)}}" class="text-blue-400 hover:text-gray-100 mx-2">
                      <i class="material-icons-outlined text-base">edit</i>
                 </a>
                  <a href="javascript:void(0)" recordid="{{$achivement->id}}" class="ConfirmDelete text-red-400 hover:text-gray-100 ml-2">
                    <svg
                    class="w-5 "
                    aria-hidden="true"
                    fill="currentColor"
  ...
  {{ $achivements->links('pagination::tailwind') }}
       ...
       Swal.fire({
         ...
         if (result.isConfirmed) {
           Swal.fire(
            ...
            window.location.href="/websiteAboteAchivement/delete/"+recordid;
    }
});    
})   
