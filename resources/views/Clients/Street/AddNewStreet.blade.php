...
<!----Form of counry  ---->
        ...
        <div class="border-collapse w-full flex flex-col pt-4 bg-gray-100">
            ...
            <form  class=" ml-5 mr-5 p-10 bg-white rounded shadow-xl" method="Post">
                @csrf
                <div class="mt-2">
                     <label class="block text-sm text-gray-600" for="">Add Street:</label><br>
                     <input  class="w-full px-2 py-2 text-gray-700 " id="" name="street_name" type="text" required="" placeholder="" >
               </div>
         ...
         <div class="flex justify-between">
             <div class="inline-flex border rounded w-7/12 px-2 lg:px-6 h-12 bg-transparent">
                ...
                <div class="align-middle inline-block min-w-full shadow overflow-hidden bg-white shadow-dashboard px-8 pt-3 rounded-bl-lg rounded-br-lg">
                    <table class="min-w-full">
                        <thead>
                            <tr>
                                ...
                                <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">Street</th>  
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-no-wrap border-b ">
                                        <div class="flex items-center">
                                            <div>
                                                <div class="text-sm leading-5 text-gray-800">{{$street->id}}</div>                                               
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-no-wrap border-b ">
                                        <div class="flex items-center">
                                            <div>
                                                <div class="text-sm leading-5 text-gray-800">{{$street->user->name}}</div>                                               
                                            </div>
                                        </div>
                                    </td>
                                  
                                    <td class="px-6 py-4 whitespace-no-wrap border-b">
                                        <div class="text-sm leading-5 text-blue-900">{{$street->city->city_name}}</div>
                                    </td>

                                    <td class="px-6 py-4 whitespace-no-wrap border-b">
                                        <div class="text-sm leading-5 text-blue-900">{{$street->street_name}}</div>
                                      </td>

                                    <td class="p-3 flex">                                      
                                        <a href="{{url('/street/edit',$street->id)}}" class="text-blue-400 hover:text-gray-100 mx-2">
                                            <i class="material-icons-outlined text-base">edit</i>
                                        </a>
                                        <a href="javascript:void(0)" recordid="{{$street->id}}" class="ConfirmDelete text-red-400 hover:text-gray-100 ml-2">
                                            <svg
                                            class="w-5 "
                                            aria-hidden="true"
                                            fill="currentColor"
                    ...
                    {{ $streets->links('pagination::tailwind') }}
       ...
       Swal.fire({
         ...
         if (result.isConfirmed) {
           Swal.fire(
            ...
            window.location.href="/street/delete/"+recordid;
