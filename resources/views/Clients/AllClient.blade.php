...
<!----table of All Client  ---->
            ...
            <div class="bg-gray-100 -my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 pr-10 lg:px-8 w-full">  
                                ...
                                <tbody>
                                 <tr >
                                    @foreach ($clients as $client)   
                                       <td class="px-4 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5" >{{$client->id}}</td>
                                       <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5" >{{$client->first_name}}</td>
                                       <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$client->last_name}}</td>
                                       <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$client->company_name}}</td>
                                       <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$client->joining_date}}</td>
                                       <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$client->email}}</td>
                                       <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$client->contact_num}}</td>
                                       <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$client->country->country_name}}</td>
                                       <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$client->city->city_name}}</td>
                                       <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$client->street->street_name}}</td>
                                       <td class="px-4 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5" >{{$client->zip}}</td>
                                       
                                        <td class="px-2 flex py-4 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">                                      
                                        <a href="{{url('/addClient/edit',$client->id)}}" class="text-blue-400 hover:text-gray-100 mx-2">
                                            <i class="material-icons-outlined text-base">edit</i>
                                        </a>
                                        <a href="javascript:void(0)" recordid="{{$client->id}}" class="ConfirmDelete text-red-400 hover:text-gray-100 ml-2">
                                            <svg
                                            class="w-5 "
                                            aria-hidden="true"
                                            fill="currentColor"
       ...
       Swal.fire({
         ...
         if (result.isConfirmed) {
           Swal.fire(
            ...
            window.location.href="/addClient/delete/"+recordid;
