          ...
          <div class="flex my-5">
            <div class="w-full  px-10 py-10">
              <div class="flex justify-between border-b pb-8">
                <h1 class="font-semibold text-2xl">Make Receipt</h1>
  ...
  <form class="p-10 rounded shadow-xl" method="Post" enctype="multipart/form-data"  >
    @csrf
      ...
      <div class="inline-block mt-2 w-1/4 pr-1">
        <label class="font-medium inline-block mb-3 text-sm uppercase">Suplier Name</label>
        <select class="p-2 text-sm w-full"  id="supliers" name="suplier_id">
      ...
      <div class="inline-block mt-2 -mx-1 pl-1 w-1/4">
          <label class=" block text-sm text-gray-600" for="">Receipt_date</label>
          <input   class="w-full px-2 py-2 text-gray-700 " id=""  name="receipt_date" type="date" required="" placeholder="" >
      </div>
              ...
              <table   class="min-w-full"> 
                  ...
                  <tbody>
                    @php
                        $sum=0;
                        $totalSquaremeters=0;
                        $amount=count($receiptcart);
                    @endphp
                                       
                      @foreach ($receiptcart as $receiptcart) 
                         @php
                            $sum = $sum +($receiptcart->quantity*($receiptcart->product->lenght/100*$receiptcart->product->width/100)*$receiptcart->product->price);
                            $totalSquaremeters=$totalSquaremeters+($receiptcart->product->lenght/100*$receiptcart->product->width/100);
                         @endphp
                      <tr >  
                          <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$receiptcart->id}}</td>
                          <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5"></td>
                          <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$receiptcart->product->design}} </td>
                          <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">
                            <div class=" quantity flex flex-row h-10 w-40 rounded-lg relative bg-transparent mt-1 px-1 py-1  mx-3">
                              ...
                              <input  class=" focus:outline-none text-center w-full bg-gray-300 font-semibold text-md hover:text-black focus:text-black  md:text-basecursor-default flex items-center text-gray-700  outline-none" name="quantity"  required="" placeholder="" value="{{$receiptcart->quantity}}">
                              <button data-cartid="{{$receiptcart->id}}"  class="btnUpdateItem  quantityMines bg-gray-300 text-gray-600 hover:text-gray-700 hover:bg-gray-400 h-full w-20 rounded-l cursor-pointer outline-none">
                                <span class="m-auto text-2xl font-thin">−</span>
                              </button>
                             
                            <button data-cartid="{{$receiptcart->id}}"  class=" btnUpdateItem  quantityPlus    bg-gray-300 text-gray-600 hover:text-gray-700 hover:bg-gray-400 h-full w-20 rounded-r cursor-pointer">
                              <span class="m-auto text-2xl font-thin">+</span>
                            </button>
                            <button  href="javascript:void(0)"  recordid="{{$receiptcart->id}}" class="ConfirmDelete bg-gray-300 text-gray-600 hover:text-gray-700 hover:bg-gray-400 h-full w-20 rounded-r cursor-pointer">
                              <span class="m-auto text-2xl font-thin">x</span>
                            </button>

                            </div></td> 
                          <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$receiptcart->product->lenght}} </td>
                          <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$receiptcart->product->width}} </td> 
                          <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{($receiptcart->product->lenght/100*$receiptcart->product->width/100)}}</td>
                          <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{$receiptcart->product->price}} $</td>     
                          <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5">{{($receiptcart->quantity*($receiptcart->product->lenght/100*$receiptcart->product->width/100)*$receiptcart->product->price)}} $</td>
                          <td class="px-2 py-2 whitespace-no-wrap border-b text-blue-900 border-gray-500 text-sm leading-5"></td>
                     </tr>
                    @endforeach 
                 </tbody>
            ...
            Swal.fire({
              ...
              if (result.isConfirmed) {
                Swal.fire(
                 ...
                 window.location.href="/receiptcart/delete/"+recordid;
 ...
 $(document).on('click','.btnUpdateItem',function(){
      ...
      //  alert(cartid);
        ...
        $.ajax({
         ...
         url:'/update-receipt-item-quantity',
         type:'post',
